...
the_basics :: proc() {
	...
	{ // The Basics
		...
		*/
		...
		fmt.println('A') // unicode codepoint literal
		fmt.print('\n')
		...
		*/
		...
		fmt.println(my_integer_variable)
		fmt.println(x)
	}
}
...
control_flow :: proc() {
	...
	{ // Control flow
		...
		for i := 0; i < 5; i += 1 do fmt.println(i)
		fmt.println("--------------------")
		...
		for value, key in some_dynamic_array {
		...
		fmt.println(ODIN_ENDIAN)
		fmt.println(ODIN_VENDOR)
		fmt.println(ODIN_VERSION)
		fmt.println(ODIN_ROOT)
		fmt.println(ODIN_DEBUG)
		fmt.println("--------------------")

		switch arch := ODIN_ARCH; arch {
		case .i386, .arm32, .wasm32:
			fmt.println("32-bit")
		case .amd64, .arm64, .wasm64:
			fmt.println("64-bit")
		case .Unknown:
			// default
	...
	{ // Defer statement
		...
		case:
			...
			/* defer if f != os.INVALID_HANDLE do os.close(f) */
			defer os.close(f)
			if err != os.ERROR_NONE { // handle error
				fmt.println("Error:", err)
				fmt.println("Handle:", f)
				break
			}
			// rest of code
			fmt.println("In error case never called")
		}
		fmt.println("--------------------")
	}
	...
	{ // Branch statements
		...
		one_step :: proc() do fmt.println("one_step")
		beyond :: proc() do fmt.println("beyond")
...
union_type :: proc() {
	...
	// More realistic examples
	{
		...
		// NOTE(mkohlhaas): How can I change values in Monster?
		...
		case Entity:
			fmt.println("I'm an Entity for sure!")
		case:
			fmt.println("I'm an Entity!")
		}
	}
	fmt.println("-------")
...
using_statement :: proc() {
...

	{
		...
		// making all the fields of position appear as if they are on Entity itself:
		Entity :: struct {
...
implicit_context_system :: proc() {
	...
	// fmt.println(c)
	// [context in core](https://github.com/odin-lang/Odin/blob/master/core/runtime/core.odin#L349)

	/*
  Context{
  allocator = Allocator{procedure = proc(rawptr, Allocator_Mode, int, int, rawptr, int, Source_Code_Location) -> ([]u8, Allocator_Error) @ 0x40DCB0, data = 0x0},
  temp_allocator = Allocator{procedure = proc(rawptr, Allocator_Mode, int, int, rawptr, int, Source_Code_Location) -> ([]u8, Allocator_Error) @ 0x430F20, data = 0x7FDE04E37720},
  assertion_failure_proc = proc(string, string, Source_Code_Location) @ 0x409130,
  logger = Logger{procedure = proc(rawptr, Logger_Level, string, bit_set[Logger_Option], Source_Code_Location) @ 0x409050, data = 0x0, lowest_level = Debug, options = bit_set[Logger_Option]{}},
  user_ptr = 0x0,
  user_index = 0,
  _internal = 0x0}
  */
	...
	what_a_fool_believes :: proc() {
		...
		// The memory management procedure uses the `context.allocator` by
...
parametric_polymorphism :: proc() {
	...
	fmt.println(double_params(1.345, 12))
	fmt.println("-------")

	{
		// Polymorphic Types and Type Specialization
		// the same for types; they are always constant; not so for procs: Table_Slot :: struct(Key, Value: typeid) {
		...
		allocate :: proc(table: ^$T/Table, capacity: int) {
			if table.allocator.procedure != nil do context.allocator = table.allocator
		...
		expand :: proc(table: ^$T/Table) {
			...
			for s in old_slots {
				if s.occupied do put(table, s.key, s.value)
			}
		}
		...
		put :: proc(table: ^Table($Key, $Value), key: Key, value: Value) {
			...
			index, ok := find_index(table, key, hash)
			if !ok {
				if f64(table.count) >= 0.75 * f64(len(table.slots)) do expand(table)
		...
		find :: proc(table: ^Table($Key, $Value), key: Key) -> (Value, bool) {
			hash := get_hash(key)
			if index, ok := find_index(table, key, hash); ok {
		...
		find_index :: proc(table: ^Table($Key, $Value), key: Key, hash: u32) -> (int, bool) {
		...
		// A better approach would either be to use
		// 1. a `u64` or equivalent for the key and let the user specify the hashing function or
		// 2. make the user store the hashing procedure with the table
	}
	...
	{ // Parametric polymorphic union
		...
		r = 123
		fmt.println(r)
		r = Error.Foo0 // r = .Foo0 is allowed too, see implicit selector expressions below
		fmt.println(r)
		fmt.println("-------")
	}

	{ // Polymorphic names
		foo :: proc($N: $I, $T: typeid) -> (res: [N + 1]T) {
...
threading_example :: proc() {
...

	{
		...
		task_proc :: proc(t: thread.Task) {
			...
			for iteration in 1 .. 5 {
				for !did_acquire(&print_mutex) do thread.yield()
		...
		pool: thread.Pool
		thread.pool_init(pool = &pool, thread_count = nThreads, allocator = context.allocator)
...

		{
			...
			for !did_acquire(&print_mutex) do thread.yield()
			thread.terminate(pool.threads[nThreads - 1], 0)
...
map_type :: proc() {
	...
	elem, ok := m["Bob"]
	if ok do fmt.println(m["Bob"])
	exists := "Bob" in m
	if exists do fmt.println(m["Bob"])
}
...
bit_set_type :: proc() {
...

	{
		...
		fmt.println(ok)
		if .Saturday in e do fmt.println("Saturday in", e)
...
quaternions :: proc() {
	...
	{ // Quaternion operations
		q := 1 + 2i + 3j + 4k
		fmt.println(typeid_of(type_of(q)))
	...
	{ // Memory layout of Quaternions
		...
		a := transmute([4]f64)q
		fmt.println(typeid_of(type_of(q)))
		fmt.println(typeid_of(type_of(a)))
...
unroll_for_statement :: proc() {
	...
	// be very useful for certain optimizations
	...
	fmt.println("Arrays")
	#unroll for elem, idx in ([?]int{1, 4, 9, 16}) {
...
where_clauses :: proc() {
	...
	{ // Parametric polymorphism checks
		...
		// Failure case: bools are not numeric.
		// i := [2]bool{true, false}
		// j := [2]bool{false, true}
...
soa_struct_layout :: proc() {
	...
	// Array of Structures (AoS), Structure of Arrays (SoA), and Array of Structures
	// of Arrays (AoSoA) refer to differing ways to arrange a sequence of data
	// records in memory, with regard to interleaving. These are of interest in SIMD
...

	{
		...
		fmt.println("\n-------------------------")
		fmt.println("## Structure of Arrays ##")
		fmt.println("-------------------------\n")
		v_soa: #soa[N]Vector3
		v_soa[0].x = 1
		v_soa[0].y = 4
		...
		fmt.println("\n---------------------------------------------------")
		fmt.println("Same syntax as AOS and treat as if it was an array:")
		fmt.println("---------------------------------------------------\n")
		fmt.println(v_soa)
		...
		fmt.println("\n--------------------------------")
		fmt.println("Can use SOA syntax if necessary:")
		fmt.println("--------------------------------\n")
		v_soa.x[0] = 1
		v_soa.y[0] = 4
		v_soa.z[0] = 9
		...
		fmt.println("\n------------------------------------------")
		fmt.println("Same pointer addresses with both syntaxes:")
		fmt.println("------------------------------------------\n")
		fmt.println(v_soa)
		assert(&v_soa[0].x == &v_soa.x[0])

		fmt.println("\n------------------")
		fmt.println("Same fmt printing:")
		fmt.println("------------------")
		fmt.println(v_aos)
		fmt.println(v_soa)
	}
	...
	{
		fmt.println("\n---------------------")
		fmt.println("soa_zip and soa_unzip")
		fmt.println("---------------------\n")
...
constant_literal_expressions :: proc() {
	...
	fmt.println(SPARSE_ENUM_ARRAY_CONST)
...
matrix_type :: proc() {
...

	{
		...
		m = matrix[2, 3]f32{1, 9, -13, 20, 5, -6}
	...
	{ // Matrices support multiplication between matrices and arrays
		m := matrix[4, 4]f32{1, 2, 3, 4, 5, 5, 4, 2, 0, 1, 3, 0, 0, 1, 4, 1}
		v := [4]f32{1, 5, 4, 3}
	...
	{ // Component-wise operations
		...
		a := matrix[2, 2]i32{1, 2, 3, 4}
		b := matrix[2, 2]i32{-5, 1, 9, -7}
